...
void setup()
{
  ...
  server.on("/getGlassStatus", HTTP_GET, []()
            { server.send(200, "application/json", String("{\"glassStatus\":") + glassStatus + "}"); });
...
{
  glassStatus = 1;
  server.send(200, "application/json", String("{\"status\":") + glassStatus + "}");
}
...
void handle_setInterval()
{
  if (server.hasArg("value"))
  {
    ...
    interval = server.arg("value").toLong();
    server.send(200, "application/json", String("{\"status\":") + glassStatus + "}");
  }
  else
  {
